...
public class DictionaryValueProvider {
	...
	public DictionaryValue getDictionaryValue(long id) {
		try {
			...
			DictionaryValue dictionaryValue = dataProvider
					.queryObject(
							"SELECT "
									+ "  dictionary_value.code as dictionary_value_code, "
									+ "  name as dictionary_value_name, "
	...
	public DictionaryValue getFullDictionaryValue(long id) {
		...
		dictionaryValue.getResources().addAll(
				getDictionaryValueResources(dictionaryValue));
		return dictionaryValue;

	}

	public Collection<DictionaryValueResource> getDictionaryValueResources(
			DictionaryValue value) {
		DataProvider dataProvider;
		try {
			dataProvider = DataProvider.instance();
			Collection<DictionaryValueResource> dictionaryValueResources = dataProvider
					.queryCollection(
							"SELECT "
									...
									+ "  resource.measure_unit as resource_measure_unit, "
									+ "  resource.type as resource_type "
									+ "  FROM dictionary_value_resource "
									+ "  INNER JOIN resource ON resource.id = dictionary_value_resource.resource_id "
									+ "  WHERE dictionary_value_id = "
									+ value.getId(),
							new DictionaryValueResourceBuilder());
	...
	public void createNewDictionaryValue(DictionaryValue dictionaryValue) {
		...
		try {
			...
			String measureUnit = createStringParameter(dictionaryValue
					.getMeasureUnit());
			dataProvider
					.update("INSERT INTO dictionary_value (dictionary_id, code, name, measure_unit) VALUES ("
							+ dictionaryId
							+ ","
							+ code
							+ ","
			...
			String dictionaryValueIdQuery = String
					.format("SELECT id  FROM dictionary_value where code=%s and name=%s and measure_unit=%s LIMIT 1",
							code, name, measureUnit);
	...
	public void updateDictionaryValue(DictionaryValue dictionaryValue) {
		if (dictionaryValue.getId() == 0) {
			throw new IllegalArgumentException(
					"Only existent dictionary value can be updated");
		}
		DataProvider dataProvider = null;
		try {
			dataProvider = DataProvider.instance();
			...
			dataProvider
					.update(String
							.format("UPDATE dictionary_value SET dictionary_id=%s, code=%s, name=%s, measure_unit=%s WHERE id = "
									+ dictionaryValue.getId(), dictionaryId,
									code, name, measureUnit));
			Collection<Long> existentResourceIds = dataProvider
					.queryCollection(
							"SELECT resource_id as id FROM dictionary_value_resource WHERE dictionary_value_id = "
									+ dictionaryValue.getId(),
							new EntityBuilder<Long>() {
								@Override
								public Long createInstance(ResultSet rs)
										throws SQLException {
			...
			for (DictionaryValueResource resource : dictionaryValue
					.getResources()) {
				if (existentResourceIds
						.contains(resource.getResource().getId())) {
					dataProvider
							.update("UPDATE dictionary_value_resource SET measure = "
									+ resource.getMeasure()
									+ " WHERE resource_id = "
									+ resource.getResource().getId()
									+ " AND "
			...
			if (resourceIdsBuilder.length() > 0) {
				dataProvider
						.update(String
								.format("DELETE FROM dictionary_value_resource WHERE dictionary_value_id=%s AND resource_id IN (%s)",
										dictionaryValue.getId(),
										resourceIdsBuilder.toString()));
			}
			dataProvider.commitTransaction();
		} catch (Exception e) {
			if (dataProvider != null) {
